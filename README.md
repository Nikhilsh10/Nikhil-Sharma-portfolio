...
## ✨ Recent Updates
- **New Projects Added**: Integrated `MediTune` (LLM fine-tuning via QLoRA) and `StreamSentinel` (Real-time Kafka/FastAPI anomaly detection).
- **ML Stack Diagram Updated**: Highlighted advanced tools like Hugging Face, LangChain, Kafka, and LightGBM in the interactive architecture visualizer.
- **Resume Alignment**: Updated professional bio and experience timeline to reflect the latest internship and MLOps focus. 
